...
export function DishCard({ item }: { item: MenuItem }) {
 ...
 <p className="line-clamp-2 min-h-10 text-sm text-muted">{item.description}</p><div className="flex items-center justify-between gap-3"><span className="font-bold">{item.has_sizes ? `From ${formatMoney(item.price)}` : formatMoney(item.price)}</span><Button aria-label={`Add ${item.name}`} size="icon" disabled={!item.is_available} onClick={handleAdd}><Plus /></Button></div></div></article>;
}
