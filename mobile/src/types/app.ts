...
export type OrderStatus =
  // A card order that exists but has not been paid yet.
  | 'PAYMENT_PENDING'
  | 'PLACED'
  | 'ACCEPTED'
  | 'PREPARING'
...
export interface AuthResponse {
  ...
  /** App this account belongs to; null for platform staff. */
...
export interface LocationScheduleOption {
  scheduled_at: string;
  label: string;
}

export interface LocationScheduleDayGroup {
  date: string;
  label: string;
  slots: LocationScheduleOption[];
}

export interface LocationScheduleOptionsResponse {
  restaurant_id: string;
  location_id: string;
  fulfillment_type: OrderFulfillmentType;
  ...
  asap_unavailable_reason: string | null;
  future_order_enabled: boolean;
  max_future_days: number;
  slot_interval_minutes: number;
  prep_buffer_minutes: number;
  scheduled_available: boolean;
  scheduled_unavailable_reason: string | null;
...
export interface PersonalizedRecommendationContext {
  ai_collection_title?: string | null;
...
/**
 * Resolved from the build's own bundle ID at startup and treated as the single
 * source of truth for this app's identity. `restaurant_id` is null for
 * marketplace apps and set for single-restaurant apps.
 */
