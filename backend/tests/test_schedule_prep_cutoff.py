...
class LastSlotLeavesTimeToCookTests(unittest.TestCase):
    ...
    def test_a_longer_delivery_eta_pulls_the_last_slot_further_back(self) -> None:
        # A long delivery ETA must NOT pull the cutoff back. The window is when
        # the shop stops taking orders; only cooking has to fit before it, and a
        # driver still out afterwards is not its problem. Reported as "delivery
        # until 8:30 PM" on a branch whose window ran to 9:30.
