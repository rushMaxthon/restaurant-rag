/**
 * The one place the admin app decides which backend it talks to.
 *
 * Every request goes through `API_BASE_URL` (services/api.ts and
 * services/aiManagerStream.ts both read it), so a deployment is retargeted by
 * changing this file or the env var below — never by editing a call site.
 *
 ...
 * pass paths relative to it (`/orders`, `/auth/login`), so a base URL without
 * the prefix produces a 404 on every single request.
 */
