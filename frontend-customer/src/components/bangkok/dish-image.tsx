...
export function DishImage({ src, name, className, priority = false }: { src: string | null; name: string; className?: string; priority?: boolean }) {
 ...
 if (!src || failed) return <div role="img" aria-label={`${name} placeholder`} className={cn("dish-placeholder flex aspect-[4/3] items-center justify-center text-3xl font-black", tone, className)}><span>{initials}</span></div>;
