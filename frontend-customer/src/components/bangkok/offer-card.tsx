...
): { to: "/menu/$itemId"; params: { itemId: string } } | { to: "/menu" } {
...
export function OfferCard({ offer }: { offer: PersonalizedOffer }) {
  ...
  return (
    <Link
      {...link}
      className="offer-card group block shrink-0 snap-start overflow-hidden rounded-xl border border-border bg-surface p-5"
    >
      ...
      <span className="offer-cta mt-4 inline-flex items-center gap-1 text-sm font-bold text-primary">
        {offer.cta_label}
