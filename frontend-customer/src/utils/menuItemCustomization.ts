...
export function toCustomizationNumber(value: DecimalValue): number {
...
export function calculateUnitPrice(input: {
  ...
  const basePrice = input.selectedSize
    ? toCustomizationNumber(input.selectedSize.price)
    : toCustomizationNumber(input.menuItem.price);
  const optionTotal = input.selectedOptions.reduce(
    (total, option) => total + toCustomizationNumber(option.extraPrice) * option.quantity,
    0,
  );
